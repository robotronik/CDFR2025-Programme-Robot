...
bool MoveColumns(int direction, int sens){//direction : droite = 1, gauche = 0 / sens 1 = up, 0 = down
    printf("MoveColumns direction : %i, sens : %i\n", direction, sens);
    int positions[2] = { direction == 0 ? 0 : 5, direction == 0 ? 13 : 6};
    if (sens == 1){//Monter les boîtes
        if (highBarrelTab[positions[0]] == 1) {printf("T'essais de monter des boite de conserve où il y en a déja\n");}
        highBarrelTab[positions[0]] = 1; highBarrelTab[positions[1]] = 1;
        lowBarrelTab[positions[0]] = 0; lowBarrelTab[positions[1]] = 0;
        lowBarrelCount -= 2;
        highBarrelCount += 2;
    }
    else if (sens == 0){//Descendre les boîtes
        highBarrelTab[positions[0]] = 0; highBarrelTab[positions[1]] = 0;
        lowBarrelTab[positions[0]] = 1; lowBarrelTab[positions[1]] = 1;
        lowBarrelCount += 2;
        highBarrelCount -= 2;
    }
    //DisplayBarrel();
    //return ArduinoMoveColums(1 ou 0) //TODO implémenter ds functions
}

//Fonction qui Gère le stockage du 2ème étage de boîtes
bool PrepareHighBarrel(int sens){//sens 1 = droite, 0 = gauche
    printf("PrepareHighBarrel sens : %i\n", sens);
    if (highBarrelCount == 0) return 1; //no columns in Highbarrel so position is good
    if (highBarrelCount >= 10) {printf("Plus de place dans le barillet 2\n"); return 1;}
    ...
    if (highBarrelCount <= 8){
        if (highBarrelTab[sens ? 7 : 12] != 1) {
            SpinBarrel(sens ? -2 : 2, 2); // Cherche un emplacement libre 
            return 0;
        }
        return 1;
    }
...
bool PrerareLowBarrel(int sens){//sens 1 = droite, 0 = gauche
    printf("PrerareLowBarrel sens : %s\n",(sens ? "droite" : "gauche"));
    if (lowBarrelCount == 0) return 1; //no columns in Lowbarrel so position is good

    if (lowBarrelCount <= 10){
        if (lowBarrelTab[sens ? 5 : 0] != 0) {
            SpinBarrel(sens ? -2 : 2, 1); // Cherche un emplacement libre de 4 cases
            return 0;
        }
        return 1;
    }
    if (lowBarrelCount == 12){
        if (lowBarrelTab[sens ? 8 : 11] != 0) {
            SpinBarrel(sens ? 2 : -2, 1); // Cherche un emplacement libre de 2 cases
            return 0;
        }
    ...
    if (lowBarrelCount == SIZE){
        if (!PrepareHighBarrel(sens)) return 0; //while HighBarrel is not ready
        MoveColumns(sens,1);
        return 0;
    }
}
...
void take(int sens){//sens 1 = droite, 0 = gauche
    if (lowBarrelCount == SIZE && highBarrelCount == 10) {printf("Plus de place dans le revolver \n"); return;}
...
bool PrepareRelease(){
    ...
    //prepare release low
    if (lowBarrelTab[4] == 1 && lowBarrelTab[5] == 0){
        SpinBarrel(-1, 1);
        return 0;
    }
...
bool ReleaseHigh(){
    printf("ReleaseHigh\n");
    if (highBarrelCount == 0) {printf("Plus de boîtes à sortir 2ème étage\n"); return 1;}
