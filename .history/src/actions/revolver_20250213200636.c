...
bool PrerareLowBarrel(int sens){//sens 1 = droite, 0 = gauche
    ...
    if (lowBarrelCount <= 10){
        ...
        return 1; //position good
    }
    if (lowBarrelCount == 12){
        if (lowBarrelTab[sens ? 8 : 11] != 0) { //while there is not a box in the last column (8 or 11)
            SpinBarrel(sens ? 2 : -2, 1);
            return 0;
        }
        ...
        MoveColumns(sens,1);
        if (!PrepareHighBarrel(sens)) return 0;
        return 1; //position good, column moved
    }

    if (lowBarrelCount == SIZE){
        if (!PrepareHighBarrel(sens)) return 0; //while HighBarrel is not ready
        MoveColumns(sens,1); //move 2 columns
        return 0; //lowBarrelCount == 12 for next iteration
    }
}
